...
def load_model_decode(data, name):
    ...
    if data.sentence_classification:
        ...
        model = SeqLabel(data)
    # model = SeqModel(data)
    # # load model need consider if the model trained in GPU and load in CPU, or vice versa #TODO: REVERT
    # if not data.HP_gpu:
    #     # model.load_state_dict(torch.load(data.model_dir))
    #     model.load_state_dict(torch.load(data.model_dir), map_location=lambda storage, loc: storage)
    #     model = torch.load(data.model_dir, map_location=lambda storage, loc: storage)
    # else:
    #     model.load_state_dict(torch.load(data.model_dir))
